...
export function AlbumScreen() {
  ...
  const pickVideo = async () => {
    try {
      ...
      if (result.assets && result.assets.length > 0) {
        ...
        console.log('选中的视频:', result.assets);
        ...
        //   } catch (saveError) {
        //     console.log('保存视频到相册失败:', saveError);
        //   }
        // }
      }
    } catch (error) {
  ...
  // video item
  const renderVideoItem = ({ item, index }: { item: MediaLibrary.Asset, index: number }) => {
    if (!item || !item.uri) {
      return null;
    }
    return (
      <>
        <TouchableOpacity style={styles.mediaItem} onPress={() => handlePreview(index, 'videos')}>
          {/* 需要展示视频封面，点击后播放视频 */}
          <Image source={{ uri: item.uri }} style={styles.mediaImage} resizeMode="cover" />
          {/* <Video source={{ uri: item.uri }} style={styles.mediaVideo} useNativeControls={true} isLooping={true} shouldPlay={true} resizeMode="contain" /> */}
        </TouchableOpacity>
      </>
    );
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerButtons}>
          ...
          <TouchableOpacity style={styles.headerButton} onPress={pickVideo}>
            <Text style={styles.headerButtonText}>🎥</Text>
          </TouchableOpacity>
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <View style={styles.emptyContainer}>
                ...
                <TouchableOpacity style={styles.emptyButton} onPress={pickVideo}>
      ...
      <Modal visible={previewVisible} transparent={true} onRequestClose={() => setPreviewVisible(false)}>
        ...
        {previewData[previewIndex] && (
          previewData[previewIndex].type === "video" ? (
            // <VideoView
            //   style={styles.previewVideo}
            //   source={{ uri: previewData[previewIndex].uri }}
            //   contentFit="contain"       // 替代 resizeMode
            //   allowsFullscreen
            //   allowsPictureInPicture
            //   isLooping
            //   shouldPlay
            //   showsControls              // ✅ 取代 useNativeControls
            // />
            // <Video
            //   source={{ uri: previewData[previewIndex].uri }}
            //   style={styles.previewVideo}
